...
1. Read the relevant contract and existing implementation before editing.
2. Identify the narrowest owner/seam; avoid unrelated refactors.
3. Preserve exact bytes and secret redaction boundaries.
4. Update README/docs when a user-visible route, configuration, or invariant changes.
5. Run focused checks, then the appropriate full checks; inspect the diff and status before committing.
6. Do not commit ignored local config, runtime data, generated `dist/`, or local collaboration notes under `team/`.
