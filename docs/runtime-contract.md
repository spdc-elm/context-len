...
## Capture mode and transport

The process-level `capture_mode` defaults to `passthrough` and is snapshotted only for each new ingress. Existing exchanges retain the mode and policy they entered with. In `passthrough`, the proxy performs same-protocol transparent forwarding of request/response application bytes (subject only to the configured authentication/header policy) and creates no session assignment, exchange/registry row, artifact, summary, catalog metadata, workspace snapshot, or workspace event. It does not run capture analysis or gate handling. `capture` is the only mode that stores artifacts, computes inspection/session projections, and applies request/response gates.

`GET /api/settings/capture` returns `{ "capture_mode": "passthrough"|"capture" }`; `PATCH` with the same field changes the mode for subsequent ingress. If either gate is `hold`, changing to `passthrough` is rejected with HTTP 409 and a stable conflict code; the gate values themselves remain explicit and unchanged. A mode change never mutates an existing exchange or feeds a derived projection into transport input. Both modes preserve the original same-protocol route and wire bytes; Responses, Chat Completions, and Anthropic Messages are never converted between protocols.

## Artifact

`ArtifactRef { artifact_id, stage, direction, content_type, content_encoding, size, sha256, complete, storage_ref }`.

Body bytes are stored externally and retrieved by artifact id. `sha256` is over exact application body bytes. Original artifacts are immutable; edits create a new artifact. The default process uses ephemeral in-memory metadata with bounded artifact memory and runtime spill files (spill files are not a restart contract). Standalone `cmd/context-lens` enables durable-local mode only when `CONTEXT_LENS_DURABLE=1`; its metadata catalog is `CONTEXT_LENS_DATA_DIR/catalog.sqlite` (default data dir `.context-lens-run`, overridable with `CONTEXT_LENS_DATA_DIR`) and private content-addressed blobs live below the data directory's `artifacts` root. SQLite stores sessions, exchanges, artifact references, blob relations, settings, and event metadata only—raw request/response bodies never enter the database. On restart, durable metadata and artifact references are hydrated lazily; missing/corrupt external blobs are reported as artifact errors rather than replaced by decoded projections. Storage capacity is exposed through `GET /api/storage` as separate `MEM` and `DISK` usage/limit counters. `MaxMemoryBytes` bounds in-process artifact bytes, while `MaxTotalBytes` bounds total artifact bytes counted across memory and disk. Capacity is reclaimed through explicit Clear Workspace or whole-session deletion.

Artifact reads are explicit and lazy. `HEAD`/metadata access does not load body bytes; range reads use `Range: bytes=start-end` (end inclusive on HTTP, translated to the store's exclusive end). Large bodies are inspected through arbitrary explicit ranges and successive lazy reads. A configured per-response chunk bound (where present) is an I/O safety detail, not a viewing or storage ceiling; neither 1 MiB nor 8 MiB limits the body that can be inspected. Full/download reads are explicit, and structural JSON/ChatML views load only the ranges they need. Search is performed through the range-capable store where available and returns bounded matches. These operations never become transport input.
...
Workspace exchange listing is metadata-only and bounded. `GET /api/exchanges` accepts `limit` and an opaque `cursor`; when more rows remain it returns `X-Next-Cursor`. Implementations must not read artifact bodies while listing or emitting events. This is a page API, not a guarantee of a complete session projection: callers/UI that have not fetched later pages cannot infer or render exchanges that are outside the loaded page or lineage. Artifact body access is lazy and explicit via `/api/artifacts/{artifact_id}` (including HEAD, arbitrary range, full/download, and bounded search behavior); full body loading is not implicit in snapshot/event requests.

`DELETE /api/exchanges` is the explicit Clear Workspace action. It clears in-flight queue work and derived session/event state, removes catalog metadata and unreferenced blobs where configured, and advances the workspace generation so callbacks from pre-clear work cannot repopulate the new workspace. `DELETE /api/sessions/{session_id}` removes one complete session ownership unit, including every turn/exchange, session metadata, and unreferenced artifacts; active exchanges are rejected safely. There is no per-turn or per-exchange deletion endpoint. `GET /api/storage` reports compact `MEM` and `DISK` usage/limits. Capacity is reclaimed explicitly through Clear Workspace or whole-session deletion.
